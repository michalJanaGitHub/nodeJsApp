...
lib.create = function (dir, file, data, callback, format=true) {
  ...
  fs.open(pom, 'wx', function (err, fileDescriptor) {
    if (!err && fileDescriptor) {
      ...
      let stringData;
      if (format) stringData = JSON.stringify(data, null, 2);
      else stringData = JSON.stringify(data);
...
lib.createA = async function (dir, file, data, format=true) {
  let err, fileDescriptor, stringData;
  if (format) stringData = JSON.stringify(data, null, 2);
  else stringData = JSON.stringify(data);
  ...
  if (err || !fileDescriptor) throw new Error('Could not create new file, it may already exist');
...
lib.readA = async function (dir, file) {
  let err, data;
  let pom = path.join(lib.baseDir, dir, file + '.json');
  [err, data] = await to(fsp.readFile(pom, 'utf8'), false);
  if (err) throw err;
  const parsedData = helpers.parseJsonToObject(data);
  return parsedData;
};
...
lib.update = function (dir, file, data, callback, format=true) {
  // Open the file for writing
  let pom = path.join(lib.baseDir, dir, file + '.json');
...
// Update data in a file
lib.updateA = async function (dir, file, data, format=true) {
  let err, fileDescriptor, stringData;

  // Open the file for writing
  let pom = path.join(lib.baseDir, dir, file + '.json');

  [err, fileDescriptor] = await to(fsp.open(pom, 'r+'));
  if (err) throw new Error('Could not open file for updating, it may not exist yet');

  [err] = await to(fsp.ftruncate(fileDescriptor));
  if (err) throw new Error('Error truncating file');

  if (format) stringData = JSON.stringify(data, null, 2);
  else stringData = JSON.stringify(data);
  [err] = await to(fsp.writeFile(fileDescriptor, stringData));
  if (err) throw new Error('Error writing to existing file');

  [err] = await to(fsp.close(fileDescriptor));
  if (err) throw new Error('Error closing existing file');
...
lib.deleteA = async function (dir, file) {
  ...
  // Unlink the file from the filesystem
  let pom = path.join(lib.baseDir, dir, file + '.json');
  [err] = await to(fsp.unlink(pom));
  if (err) throw new Error('Error unlinking file');
};
