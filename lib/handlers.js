...
let getTemplateString = (data, templateData, callback) => {
  // Reject anything that is not get
  if (data.method !== 'get') { callback(405, undefined, 'html'); return; }
  // Read in a template as a string
  helpers.getTemplateA(templateData.templateName, templateData)
    .then(str => { // Add the universal header and footer
      return helpers.addUniversalTemplatesA(str, templateData);
    }) // Return that page as HTML
    .then(str => { callback(200, str, 'html');
    })
    .catch(() => callback(500, undefined, 'html'));
};

handlers.serveSimpleTemplate = (data, callback) => {
  let templateData = {};
  ...
  getTemplateString(data, templateData, (responseCode, payload, contentType) => {
    callback(responseCode, payload, contentType);
  });
};
...
handlers._tokens.get = function (data, callback) {
  ...
  } else callback(400, { 'Error': 'Missing required field, or field invalid' });
};
