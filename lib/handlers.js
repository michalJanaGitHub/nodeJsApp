...
handlers._users.post = async function (data, callback) {
  ...
  let p = data.payload.firstName.trim();
  let firstName = typeof (p) == 'string' && p.length > 0 ? p : false;
  p = data.payload.lastName.trim();
  let lastName = typeof (p) == 'string' && p.length > 0 ? p : false;
  p = data.payload.phone.trim();
  let phone = typeof (p) == 'string' && p.length === 10 ? p : false;
  p = data.payload.password.trim();
  let password = typeof (p) == 'string' && p.length > 0 ? p : false;
  p = data.payload.tosAgreement;
  let tosAgreement = typeof (p) == 'boolean' && p === true ? true : false;
  ...
  let error, rData;
  // Make sure the user does already exist
  [error, rData] = await to(_data.readA('users', phone));
  if (!error) {
                  ...
                  userChecks.forEach(function (checkId) {
                    ...
                    _data.delete('checks', checkId, function (err) {
                      ...
                      if (checksDeleted == checksToDelete) {
                        ...
                        else callback(500, { 'Error': "Errors encountered while attempting to delete all of the user's checks. All checks may not have been deleted from the system successfully." });
                      }
                    });
                  });
                } else callback(200);
...
handlers._tokens.post = async function (data, callback) {
  let p = data.payload.phone.trim();
  let phone = typeof (p) == 'string' && p.length == 10 ? p : false;
  p = data.payload.password;
  let password = typeof (p) == 'string' && p.length > 0 ? p : false;
  if (!phone || !password) { callback(400, { 'Error': 'Missing required field(s).' }); return; }
  ...
  if (!error) callback(200, tokenObject);
...
handlers._tokens.get = async function (data, callback) {
  ...
  let p = data.queryStringObject.id.trim();
  let id = typeof (p) == 'string' && p.length === 20 ? p : false;
  if (!id) { callback(400, { 'Error': 'Missing required field, or field invalid' }); return; }
...
handlers._tokens.put = async function (data, callback) {
  let p = data.payload.id.trim();
  let id = typeof (p) == 'string' && p.length === 20 ? p : false;
  p = data.payload.extend;
  let extend = typeof (p) == 'boolean' && p === true ? true : false;

  if (!id || !extend) { callback(400, { "Error": "Missing required field(s) or field(s) are invalid." }); return; }

  // Lookup the token
  let err, tokenData;
  [err, tokenData] = await to(_data.readA('tokens', id));
  if (err) { callback(400, { 'Error': 'Specified user does not exist.' }); return; }

  // Check to make sure the token isn't already expired
  if (tokenData.expires < Date.now()) { callback(400, { "Error": "The token has already expired, and cannot be extended." }); return; }

  // Set the expiration an hour from now
  tokenData.expires = Date.now() + 1000 * 60 * 60;
  // Store the new updates
  [err] = await to(_data.updateA('tokens', id, tokenData))
  if (err) { callback(500, { 'Error': 'Could not update the token\'s expiration.' }); return; }
  callback(200);
};
...
handlers._tokens.delete = function (data, callback) {
  // Check that id is valid
  let p = data.queryStringObject.id.trim();
  let id = typeof (p) == 'string' && p.length == 20 ? p.trim() : false;
