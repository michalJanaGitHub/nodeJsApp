  ...
  if (id) {
    ...
    _data.read('checks', id, function (err, checkData) {
      if (!err && checkData) {
        ...
        _tokens.verifyToken(token, checkData.email, function (tokenIsValid) {
          // Return check data
          ...
          if (tokenIsValid) {
            ...
            _data.delete('checks', id, function (err) {
              if (!err) {
                ...
                _data.read('users', checkData.email, function (err, userData) {
                  if (!err) {
                    let userChecks = typeof (userData.checks) == 'object' && userData.checks instanceof Array ? userData.checks : [];
                    ...
                    if (checkPosition > -1) {
                      ...
                      userData.checks = userChecks;
                      _data.update('users', checkData.email, userData, function (err) {
                        if (!err) callback(200);
