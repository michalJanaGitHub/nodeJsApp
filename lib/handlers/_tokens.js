...
lib.verifyTokenA = async function (tokenId, email) {
  let err, tokenData;
  // Lookup the token
  [err, tokenData] = await to(_data.readA('tokens', tokenId));
  if (err) return false;
...
lib.getUserFromToken = async function (tokenId) {
  let err, tokenData;
  // Lookup the token
  [err, tokenData] = await to(_data.readA('tokens', tokenId));
  if (err) throw new Error('Could not find token');
  if (tokenData.expires < Date.now()) throw new Error('token expired');
  return tokenData.email;
};

// return User Email from token
lib.getUserDataFromToken = async function (tokenId) {
  let err, tokenData, userData;

  // Lookup the token
  [err, tokenData] = await to(_data.readA('tokens', tokenId));
  if (err) throw new Error('Error reading token - user may not exist');
  if (tokenData.expires < Date.now()) throw new Error('token expired');

  [err, userData] = await to(_data.readA('users', tokenData.email));
  if (err || !userData) throw new Error('Error reading userData - user may not exist');
