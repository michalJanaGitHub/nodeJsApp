...
const fs_ReadFileP = util.promisify(fs.readFile);
...
helpers.getTemplateP = (templateName, data) => {
  let promise = new Promise((resolve, reject) => {
    templateName = typeof (templateName) == 'string' && templateName.length > 0 ? templateName : false;
    data = typeof (data) == 'object' && data !== null ? data : {};
    if (!templateName) reject('A valid template name was not specified');

    let templatesPath = path.join(__dirname, '/../templates/');
    templatesPath = templatesPath + templateName + '.html';
    fs_ReadFileP(templatesPath, 'utf8')
      .then((str) => {
        if (str.length > 0) {
          let finalString = helpers.interpolate(str, data);
          resolve(finalString);
        }
        else reject('No template could be found');
      })
      .catch((err) => {
...
helpers.addUniversalTemplatesA = async function (str, data) {
  ...
  // Get the header
  [err, headerString] = await to (helpers.getTemplateP('_header', data));
  if (err) throw new Error('Could not find the header template');
  // Get the footer
  [err, footerString] = await to (helpers.getTemplateP('_footer', data));
